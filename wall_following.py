...
def get_x_y() -> Tuple[np.ndarray, np.ndarray]:
    ...
    x, y = helpers.datasets.load_wall_following()
...
def preprocess(x: np.ndarray, y: np.ndarray) -> np.ndarray:
    ...
    if PLOTTING_MODE != 'none':
        ...
        plotter.filename = 'spectral_embedding'
        plotter.xlabel = 'first feature'
        plotter.ylabel = 'second feature'
        plotter.title = 'Spectral Embedding\nn_neighbors: {}'.format(embedding_params['n_neighbors'])
        plotter.scatter(x, y, class_labels=get_wall_following_name)
